...
This structure provides rudimentary profiling control from within the program.  For more details, see \secref{profiling} and {\tt examples/profiling}.
...
\entry{\tt profile}
a compile-time constant that is true when compiling {\tt -p}.  The default is false.

\entry{\tt reset ()}
clear accumulated profiling data.  This function is equivalent to {\tt
  fn () => ()} unless the program is compiled with {\tt -p}.

\entry{\tt write f}
write accumulated profiling data to file {\tt f}.  This function is
equivalent to {\tt fn (f : string) => ()} unless the program is
compiled with {\tt -p}.  Note: a program compiled with {\tt -p} will
always write the final accumulated profiling data to a file named {\tt
  mlmon.out}.
\end{description}
...
\entry{type handler} the type of signal handlers.
...
\entry{handleWith' (s, f)} set the handler for signal {\tt s} to be the
