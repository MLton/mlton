...
examples/profiling}.  In order to most efficiently execute non-profiled
programs, when compiling {\tt -profile no} (the default), type {\tt
Data.t} is equivalent to {\tt unit} and all of the operations in {\tt
MLton.Profile} are no-ops.
...
is a no-op.  Note: a program compiled with {\tt -profile time} or {\tt
...
returns {\tt NONE}.  {\tt seed ()} may block until enough random bits
are available.
...
()} returns {\tt NONE}.  {\tt useed ()} will never block -- it will
instead return lower quality random bits.
...
unhandled exception.  It is an error for to {\tt Thread.switch'} to an
interrupted thread with a thunk that raises an exception (either
directly, or via {\tt Thread.prepend}).  This is to avoid the
possibility of aynchronous exceptions.  All signals are automatically
...
returns a handler that executes {\tt f ()} and does not change
threads.
