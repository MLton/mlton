...
The resulting executable will use a fixed size heap of size {\tt n}.  A value
...
\option{-host \{self|...\}}
Cross compile to program to run on the specified host.  The default is to
compile for the build machine.  To use any other host, you must first install a
...
not to gather profiling information.  {\tt -profile true} implies {\tt -keep
ssa}.  See \secref{profiling} for details.
