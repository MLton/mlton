...
[,shell]
----
gmake CC=$HOME/wasi-sdk-21.0/bin/clang \
      AR=$HOME/wasi-sdk-21.0/bin/ar \
      RANLIB=$HOME/wasi-sdk-21.0/bin/ranlib \
      TARGET_OS=wasi \
      TARGET_ARCH=wasm32 \
...
** Once installed, you can run MLton with the
   ``-target wasm32-unknown-wasi`` flag, to build WASI binaries:
+
[,shell]
----
mlton -target wasm32-unknown-wasi hello-world.sml
----
