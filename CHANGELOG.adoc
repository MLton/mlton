...
* 2020-01-22
  ** Added expert `-pi-style {default|npi|pic|pie}` option, which can
  be used to override a target-determined default.
...
* 2019-10-25
  ...
  ** Add `MLton.sizeAll: unit -> IntInf.int` that returns the size of
  reachable live data.
  ** Chang ``MLton.size``'s type from `'a -> int` to `'a ->
  IntInf.int`, because with 64-bit systems, the size of a single
  object can exceed that representable by a signed 32-bit integer.
...
* 2019-03-12
  ** Fix latent `MLton.GC.collect` bug; a explicit `MLton.GC.collect`
