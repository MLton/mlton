 ...
 * http://gmplib.org[GMP] (GNU Multiple Precision aritmetic library)
 * http://savannah.gnu.org/projects/make[GNU Make]
 * binutils and coreutils (`ar`, `bash`, `diff`, `find`, `git`, `grep`, `gzip`, `patch`, `sed`, `ranlib`, `tar`, ...)
...
On typical platforms, installing MLton (after performing `make all`) can be accomplished via:
 ...
 * `PREFIX`: Specify the installation prefix (e.g., `PREFIX=/opt/mlton`).
