   ...
   ("Uri.resolve", fn () =>
    ...
    let
	  ...
	  abs = toString (resolve {base = base,
       ...
       val checkResolve =
	  Trace.trace2
	  ("Uri.checkResolve", String.layout, String.layout, Bool.layout)
	  checkResolve
...
val _ =
   Assert.assert
   ("Uri", fn () =>
    fromString "mailto:sweeks@acm.org" = SOME (MailTo "sweeks@acm.org")
    andalso isSome
