...
structure Char =
   struct
	 ...
	 let val (hi, lo) = Int.divMod (ord c, radix)
	 in (fromHexDigit hi, fromHexDigit lo)
	 end
   end
...
val shouldEscape =
   Trace.trace ("Uri.shouldEscape", Char.layout, Bool.layout) shouldEscape

fun escape s =
   String.translate
   (s, fn c =>
    if shouldEscape c
       then let val (hi, lo) = Char.toHexChars c
	    in implode [#"%", hi, lo]
	    end
    else str c)
...
datatype t =
   T of {scheme: Scheme.t option, (* NONE in relative uris *)
...
val toString =
   Trace.trace ("Uri.toString", layout, String.layout) toString
...
structure Regexp =
   struct
      ...
      (*val uric = or [reserved, unreserved, escaped]*)
      (* It's pointless to follow the spec on uric, which rules out lots of
      ...
      val fragment = save (star uric, fragment')
      val query' = Save.new ()
(* The official definition of query says uric*, but this doesn't work with
 * our expanded meaning of uric = printable, since then the query consumes
 * the fragment.
 *)
(*      val query = save (star uric, query') *)
	 ...
	 save (star (isChar (fn c => (Char.isPrint c andalso
      ...
      val uricNoSlash = or [unreserved, escaped, oneOf ";?:@&=+$,"]
      val opaquePart' = Save.new ()
      val opaquePart = save (seq [uricNoSlash, star uric], opaquePart')
	 ...
	 seq [or [netPath, absPath, relPath,
		  null (* null added for empty urls -- these are
			...
			*)
			    ...
			    optional (seq [char #"?", query])]
      val absoluteUri = seq [scheme, char #":", or [hierPart, opaquePart]]
      val uri = seq [optional (or [absoluteUri, relativeUri]),
		     optional (seq [char #"#", fragment])]
      val uri = Promise.lazy (fn () => compileDFA uri)
...
fun fromString (uriString: string): t option =
   Option.map (Regexp.Compiled.matchAll (Regexp.uri(), uriString), getMatch)
   
val fromString =
   Trace.trace ("Uri.fromString", String.layout, Option.layout layout)
   fromString
