      ...
      fun getOperands (xs: Var.t vector): Operand.t vector =
         Vector.map (xs, fn x =>
            ...
            case (#operand (varInfo x)) of
                 NONE => (Error.bug o toString o seq) [str "AllocateRegisters.getOperands_1", Var.layout x]
               | SOME r =>
                  (case !r of
                       NONE => (Error.bug o toString o seq) [str "AllocateRegisters.getOperands_2", Var.layout x]
                     | SOME oper => oper)
            end)
      val getOperands =
         Trace.trace 
