 ...
 *)
functor ConstantPropagation (S: CONSTANT_PROPAGATION_STRUCTS)
	 ...
	 in case !r of
	  ...
	  | NotComputed =>
		  ...
		  val g =
			...
			Const (Const.T {const, ...}) =>
			   (case !const of
			       Const.Const c => yes (PrimExp.Const c)
			     | _ => No)
		      | Datatype (Data {value, ...}) =>
			   (case !value of
			       ConApp {con, args, ...} =>
...
fun simplify (program as Program.T {datatypes, globals, functions, main})
   ...
   let
	 ...
	 (Con.plist,
      ...
      val conArgValues = #values o conInfo
	 ...
	 fun makeDataUnknown arg: unit =
		...
		fun doit () =
		   (value := Unknown
		    ...
		    ; (List.foreach
		       (!filters, fn {con, args} =>
			coerces {froms = conArgValues con,
				 tos = args})))
	     in case !value of
		Unknown => ()
	      | Undefined => doit ()
	 ...
	 fun primApp {prim,
	    ...
	    let
	       fun bear z =
		  if once resultVar andalso Type.isSmall resultType
		     then Birth.here z
		  else Birth.unknown ()
	       fun update (a, v) =
	 ...
	 fun filter (variant, con, args) =
	    case value variant of
		  ...
		  in case !value of
		     Undefined => save ()
		   | Unknown => coerces {froms = conArgValues con, tos = args}
	 ...
	 (fn display =>
		...
		(datatypes, fn {cons, ...} =>
		 ...
		 (cons, fn {con, ...} =>
		  ...
		  (seq [Con.layout con, str " ",
			Vector.layout Value.layout (conArgValues con)]))))
	     ; display (str "\n\nConstants:")
	     ; (Program.foreachVar
       ...
       *)
      val {new = newGlobal, all} = Global.make ()
      fun replaceVar x =
	 ...
	 let
	    fun keep () =
	       SOME {var = var, ty = ty,
		     exp = PrimExp.replaceVar (exp, replaceVar)}
	 in case (Value.global (value var, newGlobal), exp) of
	    (NONE, _) => keep ()
	  | (SOME _, PrimApp {prim, ...}) =>
      ...
      val loopBind =
	 Trace.trace ("ConstantPropagation.loopBind",
		      Dec.layoutBind,
		      Option.layout Dec.layoutBind)
	 loopBind
      val traceLoopExp =
	 Trace.trace ("ConstantPropagation.loopExp", Exp.layout, Exp.layout)
	 ...
	 let
	    ...
	    val transfer =
		  ...
		  Bug => Bug
		| Call {func, args, cont} =>
		     Call {func = func, cont = cont, args = replaceVars args}
		| Case {cause, test, cases, default} =>
		     Case {cause = cause, test = replaceVar test,
			   cases = cases, default = default}
		| Jump {dst, args} => Jump {dst = dst, args = replaceVars args}
		| Raise xs => Raise (replaceVars xs)
		| Return xs => Return (replaceVars xs)
	    val decs = 
	       List.foldr
	       (decs, [], fn (d, ds) =>
		case d of
		 ...
		 | Fun {name, args, body} =>
		      Fun {name = name,
			   args = args,
			   body = loopExp body} :: ds
		 | d => d :: ds)
	 in Exp.make {transfer = transfer,
		      decs = decs}
	 end) arg
      val functions =
