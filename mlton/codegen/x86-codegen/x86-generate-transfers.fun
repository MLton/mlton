  ...
  fun generateTransfers {chunk as Chunk.T {data, blocks, ...},
	...
	val isShort' 
	  = fn li : li => let
			    val {block = Block.T {transfer, ...}, ...} = li
			  in
			    List.forall(Transfer.nearTargets transfer,
					! o #layedOut o getLabelInfo)
			  end
	val isShort = isShort' o getLabelInfo
	...
	local
	  val loc = Counter.new 0
	  type u = {source: Label.t,
		    sourceLoopDepth: int option,
		    sourceLoc: int,
		    target: Label.t,
		    targetLoopDepth: int option,
		    targetShort: bool,
		    targetRare: bool}
	  val queue : u list ref = ref []
	  ...
	  fun compare (a : u as {source = sourceA,
				 sourceLoopDepth = sourceLoopDepthA,
				 sourceLoc = sourceLocA,
				 target = targetA,
				 targetLoopDepth = targetLoopDepthA,
				 targetShort = targetShortA,
				 targetRare = targetRareA,
				 ...},
		       b : u as {source = sourceB,
				 sourceLoopDepth = sourceLoopDepthB,
				 sourceLoc = sourceLocB,
				 target = targetB,
				 targetLoopDepth = targetLoopDepthB,
				 targetShort = targetShortB,
				 targetRare = targetRareB,
				 ...})
	    = let
		val {block = blockA as Block.T {entry = entryA, ...}, ...}
		  = getLabelInfo targetA
		val {block = blockB as Block.T {entry = entryB, ...}, ...}
		  = getLabelInfo targetB
	      in 
		lexical
		[fn () => Bool.compare(Entry.isNear entryA, Entry.isNear entryB),
		 fn () => Bool.compare(targetShortA, targetShortB),
		 fn () => Bool.compare(targetRareB, targetRareA),
		 fn () => case (sourceLoopDepthA, sourceLoopDepthB)
			    of (NONE, NONE) => EQUAL
			     | (SOME _, NONE) => GREATER
			     | (NONE, SOME _) => LESS
			     | (SOME a, SOME b) => Int.compare (a, b),
		 fn () => case (targetLoopDepthA, targetLoopDepthB)
			    of (NONE, NONE) => EQUAL
			     | (SOME _, NONE) => GREATER
			     | (NONE, SOME _) => LESS
			     | (SOME a, SOME b) => Int.compare (a, b),
		 fn () => Int.compare(sourceLocB, sourceLocA)]
	      end
	  fun gte (a, b)
	    = case compare (a, b)
		of LESS => false
	  ...
	  fun enque' x = queue := List.insert(!queue, x, gte)
	in
	  fun enque {source, target}
	    = let
		val li = getLabelInfo target
		val x = {source = source,
			 sourceLoopDepth = getLoopDepth source,
			 sourceLoc = Counter.next loc,
			 target = target,
			 targetLoopDepth = getLoopDepth target,
			 targetShort = isShort' li,
			 targetRare = isRare' li}
	      in 
		enque' x
	      end 
	  val enque
	    = fn {source, target} => (print (concat ["enque:",
						     " source = ",
						     Label.toString source,
						     " target = ",
						     Label.toString target,
						     "\n"]);
				      enque {source = source, target = target})
	  ...
	  fun deque ()
	    = (case !queue
		 of [] => NONE
		  | queue' => let
				val rec loop 
				  = fn ([] : u list, acc) => (NONE, List.rev acc)
				     | (x::queue',acc)
				     => let
					  val target = #target x
					in 
					  if isReady target
					    then (SOME target, 
						  List.appendRev (acc, queue'))
					    else loop (queue',x::acc)
					end
				val (max,queue') = loop (queue', [])
			      in
				queue := queue' ;
				max
			      end)
	...
	fun fallDefault (gef as GEF {generate,effect,fall})
	  ...
	  = let
		...
		of {layedOut = ref true, ...}
		 ...
		 => if isReady' li
		      then (case getNear(jumpInfo, target)
			       ...
			       | _ => AppendList.appends
				      [default false,
				       ...
				       generate gef
						 ...
						 falling = true,
						 unique = false}])
		      else (enque {source = label, target = target};
			    default true)
	    end
