...
*)
  ...
  fun generateTransfers {chunk as Chunk.T {data, blocks, ...},
	...
	val labelInfo as {get = getLabelInfo : Label.t -> {block: Block.t,
							   layedOut: bool ref},
			  set = setLabelInfo,
		  ...
		  => (case (!stack')
			of [] => (case Queue.deque(!queue)
				    of NONE => NONE
				     | SOME(x, queue') => (queue := queue';
							   SOME x))
		         | x::stack'_ => (stack := List.rev stack'_;
					 SOME x))
		  | x::stack_ => (stack := stack_;
				  SOME x))
	end

	fun addCompensationBlock {label, id, add}
	  = let
	      val label' = Label.new label
	      val {block as Block.T {profileInfo, ...}, ...} = getLabelInfo label
	      val profileInfo
		= ProfileInfo.add
		  (profileInfo,
		   {profileLevel = 2,
	    ...
	    in
	      ...
	      Assert.assert("pushCompensationBlock",
			    fn () => getNear(jumpInfo, label') = Count 1);
	      x86LiveTransfers.setLiveTransfersEmpty(liveTransfers, label');
	      setLabelInfo(label', {block = block, layedOut = ref false});
	      add label';
	      label'
	    end
	  ...
	  = AppendList.append
		...
		=> let
		     ...
		     fun fall_truee ()
			   ...
			   val id = Directive.Id.new ()
			   val falsee'
			     = addCompensationBlock {label = falsee,
						     id = id,
						     add = add}
			 in
			   AppendList.append
		     ...
		     fun fall_falsee ()
			   ...
			   val id = Directive.Id.new ()
			   val truee' 
			     = addCompensationBlock {label = truee,
						     id = id,
						     add = add}
			 in
			   AppendList.append
		   ...
		   in
			...
			=> let
			     ...
			     fun default ()
			       = case (isRare truee, isRare falsee)
			  ...
			  fn (k, target)
			   => let
				val target_live
				  = getLive(liveInfo, target)
				val id = Directive.Id.new ()
				val target' = addCompensationBlock 
				              {label = target,
					       id = id,
					       add = if isRare target
						       then enque
						       else push}
			      in
				AppendList.fromList
	      ...
	      of Switch {test, cases, default}
	       => let
		    ...
		    fun doitTable(cases,
			  ...
			  val idT = Directive.Id.new ()
			  val defaultT = addCompensationBlock
			                 {label = default,
					  id = idT,
					  add = if isRare default
						  then enque
						  else push}
			...
			in
			  ...
			  [if Size.lt(size, Size.LONG)
			   ...
			   if shift > 0
			     then let
				    val idC = Directive.Id.new ()
				    val defaultC = addCompensationBlock
				                   {label = default,
						    id = idC,
						    add = if isRare default
							    then enque
