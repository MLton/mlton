...
struct
  ...
  fun track memloc = ClassSet.contains(livenessClasses, MemLoc.class memloc)
       ...
       fn (operand, live)
        => (case Operand.deMemloc operand
	      of NONE => live
	       | SOME memloc
	       => if track memloc
		    then MemLocSet.add(live, memloc)
		    else live))
...
(*		(Label.plist, Property.initConst LiveSet.empty) *)
	  in
	    T liveInfo
	  end
			 ...
			 of SOME memloc
			  => List.fold
			      ...
			      fn (memloc, livenessUses)
			       => if track memloc
				    then MemLocSet.add(livenessUses, memloc)
				    else livenessUses)
			  | NONE => livenessUses)
	    in
	      doit(defs,
  ...
  structure Liveness = 
	...
	= let
	    ...
	    val defs = List.fold
			...
			fn (memloc, defs)
			 => if track memloc
			      then MemLocSet.add(defs, memloc)
			      else defs)
	  in
	    liveness {uses = uses,
		      defs = defs,
	...
	= let
	    ...
	    val live
		 ...
		 LiveSet.empty,
		 fn (target, live) 
	          => MemLocSet.union(LiveInfo.getLive(liveInfo, target), 
				     live))
	  in
