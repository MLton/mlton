...
*)
            ...
            val blocks
              = List.keepAllMap
                (labels,
                 fn label 
                  => let
                       val {block = Block.T {entry, statements, transfer},
                            reach} = get label
                     in
                       if !reach
                         then SOME 
  ...
  structure MoveHoistLivenessBlock = 
    struct
      ...
      fun moveHoist {block = LivenessBlock.T {entry, statements, transfer}}
         = let
            val {transfer,live} 
  ...
  structure CopyPropagateLivenessBlock =
    struct
      ...
      fun copyPropagate {block = LivenessBlock.T {entry, statements, transfer},
                         liveInfo}
        = let
      ...
      local
        ...
        val rewriter : rewriter
          = fn {entry,
            ...
            | _ => Error.bug "amd64Simplify.PeepholeBlock: elimALRedundant"
